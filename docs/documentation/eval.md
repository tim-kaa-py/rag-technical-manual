...
_Last updated: 2026-08-02 · reflects milestone M5 (post-M5 state; staleness pass per D27 review)_
...
existed** (D17) — locked before the first measurement, though drafted knowing
the M1 smoke results, so the set is frozen-before-results but **not blind**
(every report header says so). Every expected page and golden answer was
...
- **Question types probe distinct failure modes**: lexical lookup, semantic
  ...
  table extraction, numeric limits, and an identifier lookup (`DKG-090`)
  included as D12's sparse-wins case — measured outcome: dense already
  resolved it at rank 1, so sparse-wins stayed unconfirmed at n=2 (D23).
- **Three predicted-fail questions** (q7 V-belt interval, q8 slow-crank causes,
  ...
  (D9/D19/D6). They stay in the aggregate, annotated — they established the
  pre-M5 ceiling (best achievable correct = 8/11) that M5's vision captions
  lifted (D25): the 2026-07-17 multimodal run measured 11/11 with all four
  axes at ceiling, so the set now works as a regression detector only.
