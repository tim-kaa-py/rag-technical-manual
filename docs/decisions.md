...
**Measurement rules (frozen with this decision):** trap question excluded from retrieval metrics (own row: refused yes/no); p. 48 question stays in the aggregate annotated "predicted fail — D9"; every measured config delivers exactly 5 chunks (dense@5 / fused@5 / reranked@5, three-column comparison + D3 embedding A/B); expected-page sets and hand-written golden answers frozen before results exist (later fixes go through this log); judge calibrated once by a 3× flip-rate run (0 flips → single-run thereafter, else majority-of-3); retrieved context logged at generation time and judged as-logged; report counts (7/9) never percentages, per-question table primary, instrument disclosure (judge model, prompt version, same-vendor limitation, sensitivity ≈ 11 points/question *(written at the n≈9 planning stage; the final set has n=11 answerable → ~9 points/question, as every report header states — reconciled 2026-08-02)*) in the header.
...
**Known limitation (stated, not hidden):** caption fidelity is secured only by a human audit of caption-vs-image at creation time; the automated harness can never see past the caption. *(Amended 2026-08-02, external review:)* the circularity extends to elicitation — the caption prompts were authored knowing the golden questions, so M5's correctness flips demonstrate the end-to-end mechanism over golden-tailored prompts, not general-purpose captioning quality.

## D27 — Public packaging: results in the README, learnings essay, plans kept and framed (2026-08-02)

**Decision:** Package the repo as a public skills-demonstration piece, per a three-persona external review (hiring-scan / eval-audit / narrative lenses, all "yes-with-fixes"): (1) the README leads with the measured results arc (0.79 → 0.69 → 0.91 → 1.00) plus architecture diagram and a live response example; (2) a public `docs/learnings.md` tells the build story from committed artifacts only; (3) `docs/superpowers/` plans stay tracked, framed by one README line as the plan-first AI workflow they are; (4) disclosure amendments land in the blessed M5 record (caption-prompt elicitation circularity; committed redacted candidate table making the node-identity claims publicly checkable); (5) q12's annotation wording "independent reviewer pass" is clarified to name what it was (a second, separate AI-agent review session) — annotation-only, D23 channel; (6) the `rerank_fallback_reason` follow-up from the M3 report is accepted as debt, not shipped: the eval is saturated and the flag's one occurrence (trap q12, M3) was diagnosed from console output at the time.

**Considered:**
- *Keep + frame the plans folder* — chosen: the plan→review→measure→bless loop is itself demonstration material; one framing sentence turns orchestration exhaust into evidence.
- *Delete `docs/superpowers/`* — cleaner tree and the decisions live in this log anyway, but it discards the only artifact showing *how* the build was run. Rejected.
- *Ship `rerank_fallback_reason`* — completes the instrument, but adds code churn to a saturated eval that can only detect regressions now. Rejected as not worth re-blessing.

**Why:** the review's unanimous finding was that the repo's substance outruns its packaging — the fixes surface what exists rather than adding anything new, and every disclosure gap found (elicitation circularity, unverifiable identity claim) is closed in the record itself, not in prose around it.
